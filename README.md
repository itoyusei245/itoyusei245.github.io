...
4. [ゲーム説明](#4-ゲーム説明)
   * **4.1** . [ゲーム詳細](#41-ゲーム詳細)
   * **4.2** . [ゲーム内演出](#42-ゲーム内演出)
        * **1** . [ゲーム中の副音声](#1-ゲーム中の副音声)
        * **2** . [ボスの登場シーン](#2-ボスの登場シーン)
...
### 2.空間分割
* **課題**: ステージに配置しているオブジェクトが多いのに、すべてのオブジェクトが毎フレーム Update内でプレイヤーとの衝突判定を行っていました。そのため負荷が大きくなり、フレームレートが低下する という問題が発生しました。

* **工夫**: 
    #### ８分木空間分割:
    **８分木空間分割を行いました**(時間との相談)
* **結果**: 8分木の導入により、衝突判定では「近くにある、かつ高さも近い」オブジェクトのみを高速に抽出できるようになって、判定コストを劇的に削減することに成功しました。そのため、大量のオブジェクトが存在する状況でも高フレームレートを維持できるようになりました。
